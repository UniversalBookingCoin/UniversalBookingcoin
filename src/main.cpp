...
uint256 hashGenesisBlock("0x92faf24506b3dd6d08665a70aa151f88c7e00d2b2327938179a24beab7b7e5e9");
...
const string strMessageMagic = "universalbookingcoin Signed Message:\n";
...
{
    // universalbookingcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // universalbookingcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = " UBGenesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0x8d53e3d3247ae634b4ad4e9222b2533e4fbbcb9aa3144267002a6fa8f0b5424d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd2, 0xce, 0xbf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    printf("universalbookingcoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("universalbookingcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("universalbookingcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static universalbookingcoinMiner(CWallet *pwallet)
{
    printf("universalbookingcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("universalbookingcoin-miner");
    ...
    try { loop {
        ...
        printf("Running universalbookingcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("universalbookingcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&universalbookingcoinMiner, pwallet));
}
